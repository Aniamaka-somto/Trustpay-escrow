...
export const initializePayment = async ({ email, amountKobo, reference, metadata }) => {
...
export const createTransferRecipient = async ({ name, accountNumber, bankCode }) => {
  const { data } = await axios.post(`${BASE}/transferrecipient`, {
    type: 'nuban',
    name,
...
export const initiateTransfer = async ({ amountKobo, recipientCode, reason, reference }) => {
...
export const payoutToBank = async ({ name, accountNumber, bankName, amountKobo, reference, reason }) => {
  ...
  logger.info(`Payout initiated: ${reference} → ${name} (${formatNaira(amountKobo)})`);
  return transfer;
};
...
const BANKS = {
  ...
  'sterling bank': '232', 'sterling': '232',
