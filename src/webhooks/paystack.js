...
const onChargeSuccess = async (data) => {
  ...
  if (!tx) {
    logger.warn(`Transaction ${transactionId} not found or not in PENDING state`);
    return;
  }
  ...
  await sendMessage(tx.seller.phoneNumber, msg.paymentConfirmedSeller(tx), tx.id);
};
...
const onTransferFailed = async (data) => {
  ...
  // Try to find the deal code in the reference (format: TP-PAYOUT-TP-XXXXXX-...)
  ...
  if (tx?.seller) {
    await sendMessage(tx.seller.phoneNumber,
