...
const setSession = (phone, step, data = {}) => sessions.set(phone, { step, data });
const clearSession = (phone) => sessions.delete(phone);

export const handleMessage = async (phone, text, messageType = "text", mediaId = null) => {
  ...
  try {
    ...
    if (!user.fullName) {
      if (input.length < 2) return sendMessage(phone, `Please enter your full name.`);
    ...
    if (session.step === "collecting_evidence") {
      return evidenceFlow(phone, user, input, upper, messageType, mediaId, session);
    }
    ...
    // ── SHIPMENT PROOF PHOTO ───────────────────────────────────────────────────
    if (session.step === "awaiting_shipment_proof" && mediaId) {
      const tx = await prisma.transaction.findUnique({
        where: { id: session.data.transactionId },
        include: { buyer: true },
      });
      if (tx) await saveShipmentProof(tx, user, mediaId, phone);
      return;
    }
    ...
    if (upper === "SHIPPED") return handleMarkShipped(phone, user, messageType, mediaId);

    // STATUS TP-XXXXXX
    if (upper.startsWith("STATUS ")) {
      const code = upper.replace("STATUS ", "").trim();
      return handleStatus(phone, user, code);
    }
...
const newDealFlow = async (phone, user, input, session) => {
  if (session.step === "new_deal_item") {
    if (input.length < 5) return sendMessage(phone, `Please describe the item more clearly.`);
  ...
  if (session.step === "new_deal_days") {
    ...
    if (!days || days < 1 || days > 30) return sendMessage(phone, msg.invalidDays());
...
const handleBuyerJoin = async (phone, user, dealCode) => {
  ...
  if (tx.sellerId === user.id) return sendMessage(phone, msg.cannotBuyOwnDeal());
  if (tx.status !== "PENDING") return sendMessage(phone, msg.dealUnavailable(tx.status));
...
const handleAgree = async (phone, user, session) => {
  if (session.step !== "awaiting_agree") return sendMessage(phone, msg.unknownCommand());
  const tx = await prisma.transaction.findUnique({ where: { dealCode: session.data.dealCode } });
  ...
  await prisma.transaction.update({
    where: { id: tx.id },
    data: { buyerId: user.id, paymentLinkToken: token, paymentLinkExpiresAt: paymentLinkExpiry() },
  });
  clearSession(phone);
...
const handleConfirmDelivery = async (phone, user) => {
  ...
  await sendMessage(tx.seller.phoneNumber, msg.deliveryConfirmedSeller(tx), tx.id);
};
...
const handleSellerFlag = async (phone, user) => {
  ...
  setSession(phone, "collecting_evidence", { transactionId: tx.id, disputeId: null, isSeller: true });
...
const executePayout = async (tx, seller) => {
  ...
  if (!seller.bankName || !seller.accountNumber) {
    ...
    setSession(seller.phoneNumber, "awaiting_bank_for_payout", { transactionId: tx.id });
    return;
  }
...
const handleOpenDispute = async (phone, user) => {
  ...
  setSession(phone, "collecting_evidence", { transactionId: tx.id, disputeId: null });
...
const handleSellerRespond = async (phone, user) => {
  const dispute = await prisma.dispute.findFirst({
    where: { transaction: { sellerId: user.id }, status: { in: ["OPEN", "AWAITING_SELLER"] } },
...
const evidenceFlow = async (phone, user, input, upper, messageType, mediaId, session) => {
  if (!session.data.disputeId && !session.data.isSeller) {
    const dispute = await prisma.dispute.create({
      data: { transactionId: session.data.transactionId, raisedById: user.id, reason: input, status: "OPEN" },
    });
    await prisma.user.update({ where: { id: user.id }, data: { disputesRaised: { increment: 1 } } });
    setSession(phone, "collecting_evidence", { ...session.data, disputeId: dispute.id });
    return sendMessage(phone, msg.disputeEvidencePrompt());
  }
  if (upper === "DONE") return handleEvidenceDone(phone, user, getSession(phone));
  const disputeId = session.data.disputeId;
  if (disputeId) {
    await prisma.disputeEvidence.create({
      data: { disputeId, submittedBy: user.id, type: mediaId ? "IMAGE" : "TEXT", fileUrl: mediaId || null, description: input || null },
    });
  }
...
const handleEvidenceDone = async (phone, user, session) => {
  ...
  await prisma.dispute.update({ where: { id: session.data.disputeId }, data: { status: "AI_REVIEW" } });
  clearSession(phone);
...
const handleMyDeals = async (phone, user) => {
  const [selling, buying] = await Promise.all([
    prisma.transaction.findMany({
      where: { sellerId: user.id, status: { in: ["PENDING", "FUNDED", "CONFIRMED", "DISPUTED"] } },
      orderBy: { createdAt: "desc" },
      take: 5,
    }),
    prisma.transaction.findMany({
      where: { buyerId: user.id, status: { in: ["FUNDED", "CONFIRMED", "DISPUTED"] } },
      include: { seller: true },
      orderBy: { createdAt: "desc" },
      take: 5,
    }),
  ...
  if (selling.length > 0) {
    ...
    for (const tx of selling) {
      const statusEmoji = { PENDING: "⏳", FUNDED: "💰", CONFIRMED: "🕐", DISPUTED: "⚠️" }[tx.status] || "📦";
      reply +=
        `${statusEmoji} *${tx.dealCode}*\n` +
        `   ${tx.itemDescription}\n` +
        `   ${formatNaira(tx.amountKobo)} — ${tx.status}\n` +
        (tx.status === "PENDING" ? `   _(Waiting for buyer to pay)_\n`
        : tx.status === "FUNDED" ? `   _(Deliver within ${tx.deliveryDays} day(s))_\n`
        : tx.status === "CONFIRMED" ? `   _(Buyer confirmed — payout pending)_\n`
  ...
  if (buying.length > 0) {
    ...
    for (const tx of buying) {
      const statusEmoji = { FUNDED: "📦", CONFIRMED: "🕐", DISPUTED: "⚠️" }[tx.status] || "📦";
      reply +=
        `${statusEmoji} *${tx.dealCode}*\n` +
        `   ${tx.itemDescription}\n` +
        `   From: ${tx.seller.fullName}\n` +
        `   ${formatNaira(tx.amountKobo)} — ${tx.status}\n` +
        (tx.status === "FUNDED" ? `   _(Reply *RECEIVED* when item arrives)_\n`
        : tx.status === "CONFIRMED" ? `   _(Payout processing — 1hr seller window)_\n`
        : `   _(Dispute in progress)_\n`) + `\n`;
    }
  }
...
const handleCancelDeal = async (phone, user) => {
  ...
  const list = allPending
    .map((t, i) => `${i + 1}. *${t.dealCode}* — ${t.itemDescription} (${formatNaira(t.amountKobo)})`)
    .join("\n");
...
const handleCancelSelection = async (phone, user, input, session) => {
  ...
  if (isNaN(index) || index < 0 || index >= session.data.deals.length) {
    return sendMessage(phone, `❌ Please reply with a number between 1 and ${session.data.deals.length}`);
  }
  const dealId = session.data.deals[index];
  const dealCode = session.data.codes[index];
  await prisma.transaction.update({ where: { id: dealId }, data: { status: "EXPIRED" } });
  clearSession(phone);
  sendMessage(phone, `✅ Deal *${dealCode}* has been cancelled.\n\nType *NEW DEAL* to create a new one.`);
};

// ─── RATINGS ──────────────────────────────────────────────────────────────────

const handleRating = async (phone, user, score) => {
  if (!score || score < 1 || score > 5) return sendMessage(phone, msg.invalidRating());
  const tx = await prisma.transaction.findFirst({
  ...
  const existing = await prisma.rating.findUnique({
    where: { transactionId_ratedById: { transactionId: tx.id, ratedById: user.id } },
  });
  if (existing) return sendMessage(phone, msg.alreadyRated());
  await prisma.rating.create({
    data: { transactionId: tx.id, ratedById: user.id, ratedUserId: tx.sellerId, score },
  });
  ...
  await prisma.user.update({
    ...
    data: { trustScore: Math.round((ratings._avg.score || 0) * 20), totalTransactions: { increment: 1 } },
  });
...
const handleStatus = async (phone, user, dealCode) => {
  const tx = await prisma.transaction.findUnique({
    where: { dealCode },
    include: {
      seller: true,
  ...
  // Only seller or buyer can check status
  if (tx.sellerId !== user.id && tx.buyerId !== user.id) {
    return sendMessage(phone, `❌ You are not part of deal *${dealCode}*.`);
  }
  ...
  const statusEmoji = {
    ...
    RELEASED: '✅', DISPUTED: '⚠️', REFUNDED: '↩️', EXPIRED: '❌',
  }[tx.status] || '📦';

  let timeline = `📋 *Deal Status — ${dealCode}*\n\n`;
  timeline += `${statusEmoji} Current: *${tx.status}*\n\n`;
  timeline += `📦 Item: ${tx.itemDescription}\n`;
  timeline += `💰 Amount: ${formatNaira(tx.amountKobo)}\n`;
  timeline += `👤 Seller: ${tx.seller.fullName}\n`;
  timeline += tx.buyer ? `🛒 Buyer: ${tx.buyer.fullName}\n` : `🛒 Buyer: _(not yet joined)_\n`;
  timeline += `\n*Timeline:*\n`;
  timeline += `• Deal created: ${format(tx.createdAt)}\n`;

  if (tx.fundedAt)
    timeline += `• Payment received: ${format(tx.fundedAt)}\n`;

  if (tx.shippedAt)
    timeline += `• Marked shipped: ${format(tx.shippedAt)}\n`;

  if (tx.confirmedAt)
    timeline += `• Delivery confirmed: ${format(tx.confirmedAt)}\n`;

  if (tx.releasedAt)
    timeline += `• Funds released: ${format(tx.releasedAt)}\n`;

  if (tx.dispute)
    timeline += `• Dispute opened: ${format(tx.dispute.createdAt)}\n`;

  if (tx.status === 'FUNDED' && tx.autoReleaseAt)
    timeline += `\n⏱ Auto-release: ${format(tx.autoReleaseAt)}`;

  sendMessage(phone, timeline);
};

// ─── MARK AS SHIPPED ──────────────────────────────────────────────────────────

const handleMarkShipped = async (phone, user, messageType, mediaId) => {
  // Find seller's most recent FUNDED deal
  const tx = await prisma.transaction.findFirst({
    where: { sellerId: user.id, status: 'FUNDED' },
    include: { buyer: true },
    orderBy: { fundedAt: 'desc' },
  });
  ...
  if (!tx) {
    return sendMessage(phone,
      `❌ No active deal to mark as shipped.\n\n` +
      `You can only mark a deal shipped after the buyer has paid.`
    );
  }

  // If no photo yet — ask for one
  if (!mediaId) {
    setSession(phone, 'awaiting_shipment_proof', { transactionId: tx.id });
    return sendMessage(phone,
      `📸 Please send a photo as proof of shipment.\n\n` +
      `This could be:\n` +
      `• Photo of the packaged item\n` +
      `• Courier receipt or waybill\n` +
      `• Screenshot of tracking number\n\n` +
      `_This protects you if a dispute is raised._`
    );
  }

  // Photo received — save and mark shipped
  await saveShipmentProof(tx, user, mediaId, phone);
};

const saveShipmentProof = async (tx, user, mediaId, phone) => {
  // Save the proof as dispute evidence (reuse the table)
  await prisma.disputeEvidence.create({
    data: {
      disputeId: null, // not a dispute yet — we'll link if needed
      submittedBy: user.id,
      type: 'IMAGE',
      fileUrl: mediaId,
      description: `Shipment proof for ${tx.dealCode}`,
    },
  }).catch(() => {}); // evidence table requires disputeId — store separately

  // Update transaction with shipped timestamp
  await prisma.transaction.update({
    where: { id: tx.id },
  ...
  // Notify buyer
  await sendMessage(tx.buyer.phoneNumber,
    `📦 Your item is on the way!\n\n` +
    `Deal: *${tx.dealCode}*\n` +
    `Item: ${tx.itemDescription}\n\n` +
    `The seller has marked your item as shipped and uploaded proof.\n\n` +
